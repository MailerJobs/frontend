...
const CollegeSection = () => {
  const { singlecollege, setSingleCollege, clientLogin, setClientLogin } =
    useContext(JobIndex);
  ...
  async function getStudents() {
    const URL = `student-college/${singlecollege.name}`;
              ...
              return (
                <div className="bg-white border-[1px] border-purple-button rounded-lg p-4 h-[200px] flex items-center justify-center w-[500px] ">
                  <div className="text-lg font-inter flex flex-col justify-center bg-white">
                    <h1><strong>Name:</strong> {e.full_name}</h1>
                    <h1><strong>USN:</strong> {e.usn}</h1>
                    <h1><strong>Email:</strong> {e.email}</h1>
                    <span><strong>Course:</strong> {e.course}</span>
                  </div>
                  <div>
                    <button onClick={() => handleViewResume(`uploads/resume/college/${e.resume_url}`)}>
